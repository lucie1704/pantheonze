...
<template>
  <div class="max-w-screen-xl mx-auto p-4">
    ...
    <div v-if="cartItems.length > 0" class="grid">
      <!-- Liste des produits -->
      <div class="col-12 lg:col-8">
        <div class="surface-card p-4 border-round mb-4">
          ...
          <!-- Articles -->
          <div v-for="item in cartItems" :key="item.id" class="flex align-items-center py-3 border-bottom-1 surface-border">
            <!-- Image et infos -->
            <div class="flex align-items-center flex-grow-1">
              <img :src="item.image" :alt="item.name" class="w-8rem h-6rem object-cover border-round mr-3" />
              <div>
                <h3 class="text-xl mb-2">{{ item.name }}</h3>
                <p class="text-500 mb-2">{{ item.size }}</p>
                <p v-if="item.message" class="text-500 mb-2">Message : {{ item.message }}</p>
              </div>
            </div>
      ...
      <div class="col-12 lg:col-4">
        <div class="surface-card p-4 border-round">
          ...
          <div class="mb-4">
            ...
            <small v-if="promoApplied" class="text-success">Code promo appliqué !</small>
          </div>
          ...
          <div v-if="promoDiscount > 0" class="flex justify-content-between mb-3 text-success">
            <span>Réduction</span>
